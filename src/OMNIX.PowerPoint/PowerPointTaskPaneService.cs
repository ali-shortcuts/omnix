...
namespace OMNIX.PowerPoint
{
    ...
    public sealed class PowerPointTaskPaneService
    {
        ...
        private readonly HashSet<int> _automaticAttempts = new HashSet<int>();
        private readonly HashSet<int> _creating = new HashSet<int>();
        ...
        {
            if (_disposed || key == 0 || !_creating.Add(key)) return null;
            try { return CreatePane(key, ownerWindow); }
            finally { _creating.Remove(key); }
        }

        private CustomTaskPane CreatePane(int key, object ownerWindow)
        {
