    ...
    public static class ExcelCapabilityEngine
    {
        ...
        public static void Apply(Excel.Application app,string json)
        {
            ...
            switch(id)
            {
                ...
                case "chart.create": { var r=range(); var co=sheet().ChartObjects().Add(r.Left+r.Width+20,r.Top,420,260) as Excel.ChartObject; co.Name=CapabilityArgs.S(a,"name","Chart"+sheet().ChartObjects().Count); co.Chart.SetSourceData(r); string type=CapabilityArgs.S(a,"type","column").ToLowerInvariant(); co.Chart.ChartType=type=="line"?Excel.XlChartType.xlLine:type=="pie"?Excel.XlChartType.xlPie:type=="bar"?Excel.XlChartType.xlBarClustered:Excel.XlChartType.xlColumnClustered; string title=CapabilityArgs.S(a,"title",""); if(title.Length>0){co.Chart.HasTitle=true;co.Chart.ChartTitle.Text=title;} co.Activate(); break; }
